## Hi there 👋 I'm Naomi

🎯 Aspiring Data Analyst with a Diploma In Information Technology(Software Development).
📊 Skilled in Excel, Data Analysis, and building Dashboards to generate business insights.
📍 Based in South Africa

🚀 Projects

  Retail Sales Dashboard (Excel)
  
    Analyzed retail sales data to identify trends, top-performimg products, and regional perfomance.
    
  Key insights:
  
    Gauteng generated the highest sales.
    Technology products contributed the most revenue.
    Sales increased steadily over time.
    
  Customer Churn Analysis (Excel)
  
    Analyzed customer data to identify factors contributing to customer churn.
    
  Key insights:
  
    month-to-month contracts had the highest churn rate.
    High monthly charges increased churn risk.
    Long-term contracts improved customer retention.

  🛠 Skills
    
      Microsoft Excel
      Data Analysis
      Data Cleaning
      Pivot Tables & Dashboards
      SQL
      Power BI (Learning)
  
- 📫 How to reach me: Email: naomimohlala2@gmail.com

More Projects coming soon (SQL & Power BI)

